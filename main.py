...
def cadastrarContato():
    ...
    try:
        ...
        prin("Erro na gravação do contato")
def listarContato():
...
def buscarContatoPeloNome():
    nome=input(f'digite o nome a ser procurado: ').upper() #upper recebe todos contatos maiusculo
    agenda = open("agenda.txt", "r")
    for contato in agenda:
        if nome in contato.split(";")[1].upper(): #upper recebe qualquer coisa e ler em maiusculo
            print(contato)
        agenda.close
